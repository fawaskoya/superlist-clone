...
export function TaskItem({ task, onSelect }: TaskItemProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group relative overflow-hidden border-l-4 ${priorityStyles.border} ${priorityStyles.background} backdrop-blur-sm hover:shadow-xl hover:${priorityStyles.accent} transition-all duration-300 hover:-translate-y-0.5 cursor-pointer rounded-lg mx-1 my-1 ${
        isDone ? 'opacity-60' : ''
